...
ggsave("Result/RSA_whole_model/RDMs_of_Human_and_PLM.pdf",plot = p.RDM, width = 240, height = 230, units = "mm")
rm(design,index,plot.data,RDM.matrix,RDM.visualize,k,affective,basic,appraisal)



## RSA between Humans and PLMs ----
...
RSA.total$signif = "***"
p.total = ggplot(RSA.total, aes(compare, r))+
  geom_col()+
  geom_errorbar(aes(ymin=r-se/2,ymax=r+se/2), width = 0.1)+
  geom_text(aes(label=signif), colour="black", size = 4,vjust=-1)+
  scale_x_discrete(name="",guide=guide_axis(angle = 90))+
  scale_y_continuous(name = "Kendall's tau",n.breaks = 7,expand = expansion(mult = c(0, .1)))+
  theme_classic()
ggsave("Result/RSA_whole_model/RSA_between_Human_and_PLM.pdf",plot = p.total, width = 100, height = 190, units = "mm")



## [option A] RSA with emotional features as candidates ----
...
## [option B] RSA with emotional PCs as candidates ----
# inferential statistic
RDMs.inference = list(subjective.comparison,
                      # corpus.annotation,
                      word.embedding,
...
## [option C] RSA with emotion space as candidates ----
# inferential statistic
RDMs.inference = list(subjective.comparison,
                      # corpus.annotation,
                      word.embedding,
...
## visualization: RSA between Humanss and emotional PCs ----
RSA.subjective = data.table(Attributes=factor(colnames(PCs.emotion),levels = colnames(PCs.emotion)),
                            Space=factor(c(rep("Affective Space",2),
                                         ...
                                         levels = c("Affective Space",
                                                    ...
                                                    "Appraisal Space")),
                            tau=unlist(import("Result/RSA_whole_model/RSA_with_emotional_PCs_tau.csv")[,1]),
                            se=unlist(import("Result/RSA_whole_model/RSA_with_emotional_PCs_se.csv")[,1]),
                            p=unlist(import("Result/RSA_whole_model/RSA_with_emotional_PCs_p.csv")[,1]))
RSA.subjective[,q:=p*.N/rank(p)]
RSA.subjective$signif = "n.s"
RSA.subjective[q<0.05, signif:="*"]
RSA.subjective[q<0.01, signif:="**"]
RSA.subjective[q<0.001, signif:="***"]
p.subjective = ggplot(RSA.subjective, aes(Attributes, tau, fill=Space)) +
  geom_col()+
  geom_errorbar(aes(ymin=tau-se/2,ymax=tau+se/2),
                width = 0.1, colour="black",linetype=1)+
  geom_text(aes(label=signif), colour="black", size = 4,vjust=-0.8)+
  scale_y_continuous(name = "Kendall's tau",n.breaks = 7)+
  scale_x_discrete(guide=guide_axis(angle = 90))+
  scale_fill_npg(name="")+
  theme_classic()+
  theme(legend.position=c(0.85,0.9))
ggsave("Result/RSA_whole_model/RSA_between_Human_and_emotional_PCs.pdf",plot = p.subjective, width = 180, height = 185, units = "mm")



## visualization: RSA between Humanss and emotion space ----
RSA.subjective = data.table(Attributes=factor(colnames(emotion.space)[1:3],levels = colnames(emotion.space)[1:3]),
                            Space=factor(c("Affective Space",
                                           "Basic Emotions Space",
                                           "Appraisal Space"),
                                         levels = c("Affective Space",
                                                    "Basic Emotions Space",
                                                    "Appraisal Space")),
                            tau=unlist(import("Result/RSA_whole_model/RSA_with_emotion_space_tau.csv")[1:3,1]),
                            se=unlist(import("Result/RSA_whole_model/RSA_with_emotion_space_se.csv")[1:3,1]),
                            p=unlist(import("Result/RSA_whole_model/RSA_with_emotion_space_p.csv")[1:3,1]))
RSA.subjective[,q:=p*.N/rank(p)]
RSA.subjective$signif = "n.s"
RSA.subjective[q<0.05, signif:="*"]
RSA.subjective[q<0.01, signif:="**"]
...
p.subjective = ggplot(RSA.subjective, aes(Attributes, tau, fill=Space)) +
  ...
  theme_classic()+
  theme(legend.position="none")
ggsave("Result/RSA_whole_model/RSA_between_Human_and_emotion_space.pdf",plot = p.subjective, width = 180, height = 185, units = "mm")
