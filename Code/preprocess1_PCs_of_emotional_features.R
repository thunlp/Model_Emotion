...
for (c in 1:nrow(rawdata.accept)){
  for (e in 1:27){
    ...
    ind.physical[n,"happy-face"] = as.numeric(rawdata.accept[c,(e-1)*6+2])
    ind.physical[n,"anger-face"] = as.numeric(rawdata.accept[c,(e-1)*6+3])
    ind.physical[n,"sad-face"] = as.numeric(rawdata.accept[c,(e-1)*6+4])
    ind.physical[n,"fear-face"] = as.numeric(rawdata.accept[c,(e-1)*6+5])
    ind.physical[n,"surprise-face"] = as.numeric(rawdata.accept[c,(e-1)*6+6])
    ind.physical[n,"disgust-face"] = as.numeric(rawdata.accept[c,(e-1)*6+7])
  }
}
...
colnames(PC.loading)[index.significant] = valide.PCs
...
rownames(emotion.score)=emotions
...
p1 = ggplot(plotdata, aes(x=emotion,y=features,fill=score,group=type))+
  geom_tile()+
  scale_fill_continuous(type = "viridis",name="Averaged\nrating",breaks=c(0,1),labels=c("min","max"))+
  scale_x_discrete(name="Emotions",position = "top",guide=guide_axis(angle = 90))+
  ...
  theme(panel.background=element_blank(),
        ...
        title = element_text(family = "Helvetica",size=7),
        text = element_text(family = "Helvetica",size=6))
p1
ggsave("Result/supplement_information/averaged_emotional_PCs.pdf", plot = p1, width = 120, height = 55, units = "mm")
rm(plotdata,i)
...
corr.score = cor(cbind(dist.affective,dist.physical,dist.PCs),method = "kendall")
plotdata = expand.grid(attribute1=rownames(corr.score),attribute2=colnames(corr.score))
plotdata[["r"]] = as.vector(corr.score)
library(corrplot)
corrplot(corr.score,method = "color",col = rev(COL2("RdBu",200)),tl.col = "black")
p4 = ggplot(plotdata, aes(x=attribute1,y=attribute2,fill=r))+
  geom_tile()+
...
# save RDM
for (index in c(1:ncol(dist.features))){
  RDM.matrix = matrix(0, nrow = 27, ncol = 27, dimnames = list(emotions,emotions))
  RDM.matrix[lower.tri(RDM.matrix)] = dist.features[,index]
  ...
  p.RDM = ggplot(plot.data, aes(x=x,y=y,fill=dist,color=dist))+
    geom_tile()+
    scale_fill_viridis_c(option = "cividis",name="Percentile\nof Distance",breaks=c(min(plot.data$dist),max(plot.data$dist)),labels=c("min","max"))+
    scale_color_viridis_c(option = "cividis",name="Percentile\nof Distance",breaks=c(min(plot.data$dist),max(plot.data$dist)),labels=c("min","max"))+
  ...
  ggsave(paste0("Data/preprocessed/emotional_features/",colnames(dist.features)[index],"_cividis.pdf"),plot=p.RDM, width=120, height=120, units="mm")
}
